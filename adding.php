<?php
  ...
  if(isset($_POST)){
    ...
    $sql_title_check = "SELECT title FROM movies WHERE title = '$title' ";
    if($result = @$connection ->query($sql_title_check)){
      $data = mysqli_fetch_array($result);
      $title_to_check = $data['title'];
      //var_dump($title_to_check);
      if(isset($title_to_check)){

        echo "This movie aleardy is in database!";
        ...
        echo 'Great! Thanks for adding a movie!';
      }
    }else{
      echo "ERROR: Could not able to execute $sql_title_check. " . mysqli_error($connection);
    }
      $connection ->close();
  }
}
